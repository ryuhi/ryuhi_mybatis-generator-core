...
public class ListUtilities {
    ...
    public static List<IntrospectedColumn> removeIdentityAndGeneratedAlwaysColumns(List<IntrospectedColumn> columns, List<String> insertIgnoreList) {
        List<IntrospectedColumn> filteredList = new ArrayList<IntrospectedColumn>();
        for (IntrospectedColumn ic : columns) {
        	String icName = ic.getActualColumnName();
            if (!ic.isGeneratedAlways() && !ic.isIdentity() && !insertIgnoreList.contains(icName) ) {
                filteredList.add(ic);
            }
        }
        return filteredList;
    ...
    public static List<IntrospectedColumn> removeGeneratedAlwaysColumns(List<IntrospectedColumn> columns, List<String> insertIgnoreList) {
        List<IntrospectedColumn> filteredList = new ArrayList<IntrospectedColumn>();
        for (IntrospectedColumn ic : columns) {
        	String icName = ic.getActualColumnName();
            if (!ic.isGeneratedAlways() && !insertIgnoreList.contains(icName)) {
                filteredList.add(ic);
            }
        }
        return filteredList;
