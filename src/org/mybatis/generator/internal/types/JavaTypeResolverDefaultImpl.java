...
public class JavaTypeResolverDefaultImpl implements JavaTypeResolver {
	...
	private double getJavaVersion() {
		...
		String version = props.getProperty("java.version","0").substring(0, 3);
		double ver = new Double(version);
		return ver;
    }
	
	private void usePrimitive() {
    	typeMap.remove(Types.BIGINT);
        typeMap.put(Types.BIGINT, new JdbcTypeInformation("BIGINT", //$NON-NLS-1$
                new FullyQualifiedJavaType("long")));
        typeMap.remove(Types.BIT);
        typeMap.put(Types.BIT, new JdbcTypeInformation("BIT", //$NON-NLS-1$
                new FullyQualifiedJavaType("boolean")));
        typeMap.remove(Types.BOOLEAN);
        typeMap.put(Types.BOOLEAN, new JdbcTypeInformation("BOOLEAN", //$NON-NLS-1$
                new FullyQualifiedJavaType("boolean")));
        typeMap.remove(Types.DOUBLE);
        typeMap.put(Types.DOUBLE, new JdbcTypeInformation("DOUBLE", //$NON-NLS-1$
                new FullyQualifiedJavaType("double")));
        typeMap.remove(Types.FLOAT);
        typeMap.put(Types.FLOAT, new JdbcTypeInformation("FLOAT", //$NON-NLS-1$
                new FullyQualifiedJavaType("double")));
        typeMap.remove(Types.INTEGER);
        typeMap.put(Types.INTEGER, new JdbcTypeInformation("INTEGER", //$NON-NLS-1$
                new FullyQualifiedJavaType("int")));
        typeMap.remove(Types.REAL);
        typeMap.put(Types.REAL, new JdbcTypeInformation("REAL", //$NON-NLS-1$
                new FullyQualifiedJavaType("float")));
        typeMap.remove(Types.SMALLINT);
        typeMap.put(Types.SMALLINT, new JdbcTypeInformation("SMALLINT", //$NON-NLS-1$
                new FullyQualifiedJavaType("short")));
        typeMap.remove(Types.TINYINT);
        typeMap.put(Types.TINYINT, new JdbcTypeInformation("TINYINT", //$NON-NLS-1$
    ...
    public void addConfigurationProperties(Properties properties) {
        ...
        usePrimitive = StringUtility
        		.isTrue(properties
        				.getProperty(PropertyRegistry.USE_PRIMITIVE));
        if (usePrimitive) {
        	usePrimitive();
        }
    }
